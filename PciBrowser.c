...
/*
????
- InitKeyboard(): ???? SimpleTextInEx protocol???? fallback ? ConIn?
- WaitForKey(): ?? WaitForEvent??? while-flush ?? shell ????????
- ReadKey(): ??? ready ??????????????? TRUE if ????
*/
...
InitKeyboard(VOID)
{
  ...
  gTextInEx = NULL;
  // ?? ConsoleInHandle ??? SimpleTextInEx?????
  ...
  // ???? console handle ? SimpleTextInEx??? LocateProtocol ????????
  if (EFI_ERROR(Status)) {
    Status = gBS->LocateProtocol(&gEfiSimpleTextInputExProtocolGuid, NULL, (VOID**)&gTextInEx);
    if (EFI_ERROR(Status)) {
      gTextInEx = NULL; // fallback to legacy ConIn
    }
  }
...
VOID WaitForKey(VOID) {
  UINTN Index;
  if (gTextInEx) {
    gBS->WaitForEvent(1, &gTextInEx->WaitForKeyEx, &Index);
  }
  else {
...
- ???? key??? TRUE??? Key ? ScanCode ??
- ????????? FALSE
?????????? WaitForKey() ?????? event ?? ready?
*/
STATIC
BOOLEAN ReadKey(EFI_INPUT_KEY* Key, UINT16* ScanCode) {
  EFI_STATUS Status;
  ...
  if (gTextInEx) {
    EFI_KEY_DATA kd;
    Status = gTextInEx->ReadKeyStrokeEx(gTextInEx, &kd);
    if (!EFI_ERROR(Status)) {
      if (Key) *Key = kd.Key;
    ...
    EFI_INPUT_KEY k;
    Status = gST->ConIn->ReadKeyStroke(gST->ConIn, &k);
    if (!EFI_ERROR(Status)) {
      if (Key) *Key = k;
...
EFI_STATUS DoConfigReadWrite(PCI_DEVICE_ENTRY* e, BOOLEAN WriteOp) {
  ...
  UINTN align = widthSel;  // 1, 2, 4, 8
  if ((offset % align) != 0) {
    Print(L"Error: offset 0x%lx is not %u-byte aligned!\n", offset, widthSel);
    WaitForKey();
    return EFI_INVALID_PARAMETER;
  }
