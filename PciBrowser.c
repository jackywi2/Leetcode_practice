...
VOID WaitForKey(VOID) {
  if (gTextInEx) {
    ...
    gTextInEx->WaitForKeyEx(gTextInEx);
  } else {
    EFI_INPUT_KEY k;
...
EFI_STATUS DoConfigReadWrite(PCI_DEVICE_ENTRY *e, BOOLEAN WriteOp) {
  ...
  if (!WriteOp) {
    UINT64 val = 0;
    EFI_STATUS s = e->PciIo->Pci.Read(e->PciIo, w, offset, 1, &val);
    if (EFI_ERROR(s)) { Print(L"Read failed: %r\n", s); return s; }
    Print(L"Value @CFG[0x%lx] = 0x%lx\n", offset, val);
  } else {
    UINT64 val = ReadHexFromConsole(L"Value to write", 0, &abort);
    if (abort) return EFI_ABORTED;
    EFI_STATUS s = e->PciIo->Pci.Write(e->PciIo, w, offset, 1, &val);
    if (EFI_ERROR(s)) { Print(L"Write failed: %r\n", s); return s; }
...
EFI_STATUS DoIoReadWrite(PCI_DEVICE_ENTRY *e, BOOLEAN WriteOp) {
  ...
  if (!WriteOp) {
    UINT64 val = 0;
    EFI_STATUS s = e->PciIo->Io.Read(e->PciIo, w, bar, offset, 1, &val);
    if (EFI_ERROR(s)) { Print(L"IO Read failed: %r\n", s); return s; }
    Print(L"Value @IOBAR%u[0x%lx] = 0x%lx\n", (UINT32)bar, offset, val);
  } else {
    UINT64 val = ReadHexFromConsole(L"Value to write", 0, &abort);
    if (abort) return EFI_ABORTED;
    EFI_STATUS s = e->PciIo->Io.Write(e->PciIo, w, bar, offset, 1, &val);
    if (EFI_ERROR(s)) { Print(L"IO Write failed: %r\n", s); return s; }
...
EFI_STATUS DoMmioReadWrite(PCI_DEVICE_ENTRY *e, BOOLEAN WriteOp) {
  ...
  if (!WriteOp) {
    UINT64 val = 0;
    EFI_STATUS s = e->PciIo->Mem.Read(e->PciIo, w, bar, offset, 1, &val);
    if (EFI_ERROR(s)) { Print(L"MMIO Read failed: %r\n", s); return s; }
    Print(L"Value @MMIOBAR%u[0x%lx] = 0x%lx\n", (UINT32)bar, offset, val);
  } else {
    UINT64 val = ReadHexFromConsole(L"Value to write", 0, &abort);
    if (abort) return EFI_ABORTED;
    EFI_STATUS s = e->PciIo->Mem.Write(e->PciIo, w, bar, offset, 1, &val);
    if (EFI_ERROR(s)) { Print(L"MMIO Write failed: %r\n", s); return s; }
